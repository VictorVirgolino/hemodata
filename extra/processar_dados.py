...
dicionario_df["nome_original_limpo"] = dicionario_df["nome_original"].apply(
    clean_col_name
)
...
files = [
    f
    for f in os.listdir(dados_brutos_path)
...
for file in files:
    ...
    try:
        ...
        # Adicionar coluna 'uf'
        # Extrai a UF do nome do arquivo de forma mais segura
        match = re.search(r"_([a-zA-Z]{2})\.", file)
        uf = match.group(1) if match else file.split(".")[0]
        df["uf"] = uf

        # Imprime o cabeçalho da coluna 'uf' para verificação
        print(f"\nVerificando 'uf' para o arquivo: {file}")
        print(df["uf"].head())
